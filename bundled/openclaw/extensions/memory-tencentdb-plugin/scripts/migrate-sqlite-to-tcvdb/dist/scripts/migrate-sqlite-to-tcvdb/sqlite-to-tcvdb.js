...
import { listLocalProfiles } from "../../src/profile/profile-sync.js";
import { createBM25Encoder } from "../../src/store/bm25-local.js";
import { VectorStore } from "../../src/store/sqlite.js";
import { TcvdbMemoryStore } from "../../src/store/tcvdb.js";
...
export async function collectMigrationPreflight(options) {
    await ensureReadableDirectory(options.pluginDataDir, "plugin data directory");
...
export async function runMigrationCli(argv, deps = {}) {
    ...
    try {
        ...
        const migration = {
            l1Migrated: options.layers.includes("l1") ? await migrateL1Records(sourceStore, targetStore, pageSize) : 0,
            l0Migrated: options.layers.includes("l0") ? await migrateL0Records(sourceStore, targetStore, pageSize) : 0,
            profileMigrated: options.layers.includes("l2") || options.layers.includes("l3")
