...
async function searchByKeyword(
  ...
  // 设计说明：
  // 1) 当 FTS5 不可用，或者 FTS5 无命中时，启用 JSONL 兜底；
  // 2) 兜底只在 recall(keyword/hybrid 的 keyword 支路)触发，避免“完全无记忆”；
  // 3) 评分使用轻量 token 覆盖率 + 命中密度，和 tool 侧 memory-search 保持一致思路。
  const cleanQuery = sanitizeText(userText).trim();
  if (!cleanQuery) {
    logger?.debug?.(`${TAG} [keyword-jsonl] query empty after sanitize, skip fallback`);
    return [];
  }

  // 先尝试复用 FTS query 的 token（保持口径一致），失败再退化到 Unicode 分词。
  const ftsLikeQuery = buildFtsQuery(cleanQuery);
  const tokens = extractKeywordTokensForRecall(cleanQuery, ftsLikeQuery);
  if (tokens.length === 0) {
    logger?.debug?.(`${TAG} [keyword-jsonl] no usable tokens, skip fallback`);
    return [];
  }

  // 读取全量 L1 JSONL 记录作为召回兜底数据源。
  // 说明：这里不带 session 过滤，是为了保持和 FTS 主路径一致的“全局记忆召回”语义。
  const records = await readAllMemoryRecords(pluginDataDir, logger);
  if (records.length === 0) {
    logger?.debug?.(`${TAG} [keyword-jsonl] no local records found`);
    return [];
  }

  const scored = records
    .map((record) => {
      // 拼接可检索字段：内容 + scene + type，提高关键词匹配召回率。
      const haystack = `${record.content} ${record.scene_name} ${record.type}`.toLowerCase();
      const score = scoreByKeywordTokensForRecall(haystack, tokens);
      if (score <= 0) return null;
      return { record, score };
    })
    .filter((item): item is { record: MemoryRecord; score: number } => !!item)
    .sort((a, b) => b.score - a.score)
    .slice(0, maxResults);

  if (scored.length === 0) {
    logger?.debug?.(`${TAG} [keyword-jsonl] fallback returned 0 hits`);
    return [];
  }

  logger?.debug?.(
    `${TAG} [keyword-jsonl] fallback hits=${scored.length}/${records.length}, tokens=${tokens.length}`,
  );
  return scored
    .filter((item) => item.score >= Math.min(threshold, 0.05))
    .map((item) => formatMemoryLine(recordToFormatable(item.record)));
}
...
/**
 * recall 侧 JSONL fallback 的 token 提取：
 * 1) 优先从 FTS 查询串（如 `"a" OR "b"`）还原 token；
 * 2) 若 FTS 串不可用，则回退到 Unicode 词元切分。
 */
function extractKeywordTokensForRecall(cleanQuery: string, ftsLikeQuery: string | null): string[] {
  if (ftsLikeQuery) {
...
/**
 * recall 侧 JSONL fallback 评分函数：
 * - coverage: 命中 token 比例（主信号）；
 * - density: 命中次数的对数缩放（次信号）。
 */
function scoreByKeywordTokensForRecall(haystackLower: string, tokens: string[]): number {
