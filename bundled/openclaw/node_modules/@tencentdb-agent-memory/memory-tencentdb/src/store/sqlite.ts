...
export class VectorStore implements IMemoryStore {
  ...
  /**
   * sqlite-vec 扩展是否成功加载。
   * true: 允许创建 vec0 表；false: 仅启用 SQLite 元数据与 FTS 路径。
   */
  private vecExtensionReady = false;
   ...
   * 2) sqlite-vec 扩展加载成功。
   */
  private canUseVectorTables(): boolean {
    return this.dimensions > 0 && this.vecExtensionReady;
  }
  ...
  init(providerInfo?: EmbeddingProviderInfo): VectorStoreInitResult {
    ...
    try {
      ...
      this.vecExtensionReady = true;
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      this.logger?.warn(
        `${TAG} Failed to load sqlite-vec extension: ${message}. ` +
        `Will continue with SQLite metadata + FTS only (vector search disabled).`,
      );
      // 不进入 degraded，确保 FTS 与关系表仍然可用。
      this.vecExtensionReady = false;
    }
  ...
  private initSchema(providerInfo?: EmbeddingProviderInfo): VectorStoreInitResult {
    ...
    // 仅在向量条件满足时标记可用。
    this.vecTablesReady = this.canUseVectorTables();
    if (!this.vecTablesReady) {
      this.logger?.info(
        `${TAG} Vector tables disabled (dimensions=${this.dimensions}, vecExtensionReady=${this.vecExtensionReady})`,
      );
    }
