...
export function createStoreBundle(
  ...
  switch (config.storeBackend) {
    ...
    default: {
      ...
      if (config.embedding.enabled) {
        if (config.embedding.provider === "local") {
          embeddingService = createEmbeddingService(
            {
              provider: "local",
              ...(config.embedding.model ? { modelPath: config.embedding.model } : {}),
              ...(config.embedding.modelCacheDir ? { modelCacheDir: config.embedding.modelCacheDir } : {}),
            },
            logger,
          );
      ...
      // Prefer runtime embedding service dimensions (e.g. local=768), fallback to config.
      // provider="none" keeps dims=0 so vec0 tables remain deferred.
      const dims = embeddingService?.getDimensions() ?? config.embedding.dimensions;
