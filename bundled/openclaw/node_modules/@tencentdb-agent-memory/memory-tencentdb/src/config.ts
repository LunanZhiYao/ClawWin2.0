...
export function parseConfig(raw: Record<string, unknown> | undefined): MemoryTdaiConfig {
  ...
  // provider="local" → use node-llama-cpp local embedding (fully offline)
  ...
  const defaultModel =
    embeddingProvider === "none" ? "" :
    embeddingProvider === "local" ? "embeddinggemma-300m-qat-q8_0" :
    embeddingModelRaw;
  ...
  return {
    ...
    recall: {
      ...
      // 默认改为 keyword：优先 FTS，失败时再走 JSONL fallback。
      strategy: validateStrategy(str(recallGroup, "strategy")) ?? "keyword",
