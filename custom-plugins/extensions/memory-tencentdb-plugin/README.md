...
> ClawWin 内置版已针对 `openclaw@2026.2.14` 做兼容适配。
...
- ClawWin 安装向导会默认启用 `memory-tencentdb`，默认后端为本地 `sqlite`。
- 客户端数据默认落在用户目录：`~/.openclaw/memory-tdai/`，卸载程序不会自动删除该目录。
- 建议将 `~/.openclaw/` 纳入客户机器备份策略（至少备份 `memory-tdai/` 与 `openclaw.json`）。
- 需要切换到 TCVDB 时，仅需在 `~/.openclaw/openclaw.json` 将 `storeBackend` 改为 `tcvdb` 并补齐 `tcvdb.url` / `tcvdb.apiKey`。
- 若更新后发现“召回失效”，先检查 `plugins.entries.memory-tencentdb.hooks.allowPromptInjection` 是否被误设为 `false`。
